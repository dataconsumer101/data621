...
long_train <- gather(train, metric, value)
...
right_skewed <- c('team_baserun_cs', 'team_baserun_sb', 'team_batting_3b', 'team_fielding_e', 'team_pitching_bb', 'team_pitching_h','team_pitching_so') 

# log scale for right skewed distributions
filter(long_train, metric %in% right_skewed) %>%
  ggplot(aes(x = value)) +
  geom_density() +
  facet_wrap(~metric, scales = 'free') +
  scale_x_log10()
...
ggplot(preds, aes(x = target_wins, y = val)) +
  geom_point(alpha = .1) +
...
lm5 <- lm(target_wins ~ team_batting_h + team_pitching_h + team_pitching_bb + team_fielding_e + team_fielding_dp, data = train)

summary(lm5)

lm6 <- lm(target_wins ~ team_fielding_e + team_fielding_dp +
...
lm7 <- lm(target_wins ~ team_fielding_e + team_fielding_dp +
...
lm8 <- lm(target_wins ~ team_batting_h + team_pitching_h + team_pitching_bb + team_fielding_e + team_fielding_dp, data = train)
